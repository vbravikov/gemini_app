...
[ ]Edit after logging — the "Edit" button in the meal-info action bar is a no-op. Once a meal is logged, there's no way to change it.
...
[ ]Per-ingredient editing — the AI's ingredient weights are guesses. There's no way to correct "Chicken Breast: 150g" to the actual portion.
...
[ ]Confidence indicator — the AI's confidence: "low"/"medium"/"high" only shows as a small text label in the hero. A low-confidence analysis should show a more prominent warning so the user doesn't blindly trust bad data.
...
Code / Architecture
[ ]theme prop drilling — useTheme() is called at the screen level and passed down through every component as a plain any prop. This is verbose and bypasses TypeScript safety. A ThemeContext (or a typed hook used locally) would be cleaner.
